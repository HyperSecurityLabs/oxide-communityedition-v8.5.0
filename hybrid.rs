...
use oxide::advanced::cache::{ScanCache, ScanCheckpoint};
...
use oxide::scanner::hypersecurity_cf::{HyperSecurityCf, WafBypassSession};
use crate::
...
pub struct HybridScanner {
    ...
    cf_bypass: Option<HyperSecurityCf>,
    cf_session: std::sync::Mutex<Option<WafBypassSession>>,
...
impl HybridScanner {
    pub fn new(args: CliArgs) -> Result<Self> {
        Self::new_with_shutdown(args, Arc::new(AtomicBool::new(false)))
    }

    pub fn new_with_shutdown(args: CliArgs, shutdown: Arc<AtomicBool>) -> Result<Self> {
        ...
        // If --resume, try to load checkpoint
        if args.resume {
            let target_url = args.target_url().to_string();
            if let Some(checkpoint) = Self::load_existing_checkpoint(&scan_id, &target_url) {
                println!("  {} Resuming scan {} from checkpoint",
                    "◈".truecolor(0, 180, 120),
                    &checkpoint.scan_id);
                ...
                println!("  {} No checkpoint found — starting fresh scan",
        ...
        // Initialize cache synchronously (we're outside async context)
        let cache = if std::path::Path::new("checkpoints").exists() || std::fs::create_dir_all("checkpoints").is_ok() {
            Some(Arc::new(ScanCache::new_sync("checkpoints")))
        } else {
            None
        };
        ...
        Ok(Self {
            ...
            signature_db,
            zero_day_engine,
            cf_bypass: HyperSecurityCf::new().ok(),
            cf_session: std::sync::Mutex::new(None),
    ...
    pub fn get_discovered_urls(&self) -> Vec<String> {
        self.completed_urls.clone()
    }

    pub fn should_continue(&self, start: std::time::Instant, duration_limit: Option<std::time::Duration>) -> bool {
    ...
    fn load_existing_checkpoint(scan_id: &str, target_url: &str) -> Option<ScanCheckpoint> {
        let dir = std::path::Path::new("checkpoints");
        if !dir.is_dir() {
            return None;
        }
        let entries = std::fs::read_dir(dir).ok()?;
        let mut best: Option<ScanCheckpoint> = None;
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map(|e| e == "cache").unwrap_or(false) {
                if let Ok(data) = std::fs::read(&path) {
                    if ScanCache::validate_checkpoint(&data) {
                        if let Ok(cp) = serde_json::from_slice::<ScanCheckpoint>(&data) {
                            // Prefer checkpoint matching target URL
                            if cp.target == target_url {
                                return Some(cp);
                            }
    ...
    pub async fn save_checkpoint(&self, current_findings: &[Finding]) {
        ...
        // Build scan config from current args
        let mut scan_config = std::collections::HashMap::new();
        scan_config.insert("target".to_string(), self.args.target_url().to_string());
        scan_config.insert("duration".to_string(), self.args.duration.to_string());
        scan_config.insert("threads".to_string(), self.args.threads.to_string());
        scan_config.insert("headless".to_string(), self.args.headless.to_string());
        if let Some(ref proxy) = self.args.proxy {
            scan_config.insert("proxy".to_string(), proxy.clone());
        }

        let checkpoint = ScanCheckpoint {
            scan_id: self.scan_id.clone(),
            target: self.args.target_url().to_string(),
            start_time: now,
            last_update: now,
            completed_urls: self.completed_urls.clone(),
            pending_urls: self.pending_urls.clone(),
            completed_phases: self.completed_phases.clone(),
            findings: current_findings.to_vec(),
            scan_config,
        };

        if let Err(e) = cache.create_checkpoint(&checkpoint).await {
    ...
    pub async fn run_hybrid_scan(&mut self) -> Result<Vec<Finding>> {
        ...
        let mut all_findings: Vec<Finding> = self.findings.clone();
        ...
        macro_rules! check_timeout {
            () => {
                if self.is_shutting_down() {
                    println!("  {} Shutdown requested — stopping scan",
                        tc("[!]", COL_CRIT));
        ...
        if !excluded.contains(&"fingerprint".to_string()) {
            ...
            if self.args.active {
                ...
                macro_rules! recon_step {
                    ($label:expr, $work:expr) => {{
                        ...
                        print!("\r\x1B[2K");
                        println!("  [38;2;80;220;160m✓\x1B[0m  [38;2;100;210;255m{}\x1B[0m", $label);
                        result
                    }};
                }
                ...
                println!("  [38;2;0;180;120m[+]\x1B[0m[38;2;0;180;120mRECON[0m  [38;2;100;210;255m[{:02}:{:02}]\x1B[0m",
                ...
                if !cf_bypass.is_empty() {
                    ...
                    for (header, server, body) in &cf_bypass {
                        let note = if !server.is_empty() && !server.to_lowercase().contains("cloudflare") {
                            ...
                            " [38;2;100;210;255m(blocked by CF)\x1B[0m".to_string()
                        } else {
                            String::new()
                        };
                        if !server.is_empty() || !note.is_empty() {
                            println!("    [38;2;100;210;255m{:<30}\x1B[0m [38;2;220;200;255m{:<20}\x1B[0m{}", header, server, note);
                        }
                    }
            ...
            let mut found_waf = String::new();
            let mut found_server = String::new();
            let mut found_framework = String::new();
            ...
            if let Ok(resp) = self.client.get(self.args.target_url()).await {
                ...
                if let Some(powered) = resp.powered_by() {
                    ...
                    found_framework = powered.to_string();
                }
                // Cookie / Set-Cookie
            ...
            let srv = if found_server.is_empty() { "-".to_string() } else { found_server };
            let waf = if found_waf.is_empty() { "-".to_string() } else { found_waf };
            let fw  = if found_framework.is_empty() { "-".to_string() } else { found_framework };
            let db  = if found_db.is_empty() { "-".to_string() } else { found_db };
            println!("  {}recon  {}server {}  {}waf {}  {}fw {}  {}db {}",
                jade, dim, srv, dim, waf, dim, fw, dim, db);
            }
        }

        // Checkpoint after Phase 1
        if !self.completed_phases.contains(&"recon".to_string()) {
            self.completed_phases.push("recon".to_string());
            self.save_checkpoint(&all_findings).await;
        }

        // ── Cloudflare WAF auto-detect + bypass configuration ─────────────
        // Runs during recon so bypass is ready before any fuzzing/scanner
        // phases. If CF is not present the session stays empty.
        if let Some(ref cf) = self.cf_bypass {
            if cf.detect_cf(self.args.target_url()).await {
                let session = cf.auto_configure(self.args.target_url()).await;
                if session.cf_detected {
                    let strat = &session.current_strategy;
                    let score = session.strategy_scores.get(strat.name()).copied().unwrap_or(0.0);
                    println!("  {} Cloudflare bypass ready — strategy: {} (confidence: {:.2})",
                        tc("[+]", OSAKA_JADE_B),
                        strat.name(),
                        score);
                    *self.cf_session.lock().unwrap() = Some(session);
                }
            }
        }
        ...
        check_timeout!();
        if !self.should_continue(start, duration_limit) {
            return Ok(all_findings);
        }
        let mut crawled_urls: Vec<String> = Vec::new();
        // On resume, skip completed phases
        if self.completed_phases.contains(&"crawl".to_string()) {
            println!("  {} Phase 'crawl' already completed — skipping",
                tc("▶", LAVENDER_BLUE));
            // Restore crawled URLs from checkpoint data
            crawled_urls = self.completed_urls.clone();
            if crawled_urls.is_empty() {
                crawled_urls.push(Parser::ensure_http(self.args.target_url()));
            }
        } else if !excluded.contains(&"crawl".to_string()) {
            println!("  {} {}  {} {}  {}",
                tc("─", OSAKA_JADE),
        ...
        let _downloader = if self.args.download {
            ...
            println!(
                "  DOWNLOAD  Auto-download enabled → [38;2;100;210;255m{}\x1B[0m",
                dl.base_dir().display()
            );
            Some(dl)
        } else {
        ...
        if !self.completed_phases.contains(&"crawl".to_string()) {
            self.completed_urls = crawled_urls.clone();
            self.completed_phases.push("crawl".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        if self.completed_phases.contains(&"fuzz".to_string()) {
            println!("  {} Phase 'fuzz' already completed — skipping",
                tc("▶", LAVENDER_BLUE));
        } else {
                ...
                if !self.args.verbose {
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ...
                            let tick = tick_frames[frame_idx % 4];
                            let tick_colors = [RP_GOLD, LIGHT_CYAN, OSAKA_JADE_B, OSAKA_JADE];
                            let tick_color = tick_colors[frame_idx % 4];
                            let spinner = format!("{}[{}]",
                                tc(&format!("[{}{}]", cw, ccw), LIGHT_CYAN),
                                tc(&tick, tick_color));
                            frame_idx += 1;
                            ...
                            let border = tc("│", LAVENDER_BLUE);
                            let _lock = sl.lock().unwrap_or_else(|e| e.into_inner());
                            if !first_render {
                                print!("\x1B[3A");
                            } else {
                                // Print 3 blank cleared lines to push banner artifacts out
                                for _ in 0..3 {
                                    println!("\r\x1B[2K");
                                }
                                print!("\x1B[3A");
                            ...
                            println!("\r\x1B[2K {} [{}] {} {}% {:>4}/{} det:{} err:{} {:.1}s {}",
                                &spinner,
                                tc(&blocks, LAVENDER_BLUE),
                                tc(&pip, LIGHT_CYAN_B),
                                tc(&format!("{:>5.1}", pct * 100.0), OSAKA_JADE_B),
                                req, total,
                                tc(&det.to_string(), LAVENDER), tc(&err.to_string(), COL_CRIT),
                                secs,
                                border);
                            // Line 2: URL being fuzzed (middle)
                            println!("\r\x1B[2K {} {} {}",
                                tc("[▶]", LIGHT_CYAN),
                                tc(&cur_fuzz, LIGHT_CYAN),
                                border);
                            // Line 3: status summary (bottom)
                            let status = format!("{} req · req/s:{:.0}", req, rate);
                            println!("\r\x1B[2K {} {} {}",
                                tc(&cursor, LAVENDER_BLUE),
                                tc(&status, LIGHT_CYAN),
                                border);
                            let _ = std::io::Write::flush(&mut std::io::stdout());
                            drop(_lock);
                        }
        ...
        } // end else (resume skip)

        // Checkpoint after Phase 3
        if !self.completed_phases.contains(&"fuzz".to_string()) {
            self.completed_phases.push("fuzz".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        if modules.contains(&"all".to_string()) || modules.contains(&"sqli".to_string()) {
            if !excluded.contains(&"sqli".to_string()) {
                ...
                for url in crawled_urls.iter().take(self.args.payload_limit) {
                    check_timeout!();
                    if !self.args.verbose {
                        println!("  [38;2;100;210;255m->\x1B[0m {}", url);
                    ...
                    if let Ok(findings) = sqli_scanner.comprehensive_scan(url, &params).await {
                        for finding in findings {
                            ...
                            if !self.args.verbose {
                                println!("  {} {}  [38;2;100;210;255m{}\x1B[0m", fmt_sev_label(&f.severity), f.title, url);
                                ph_lines.fetch_add(1, Ordering::Relaxed);
                            } else {
                                println!("  SQLi  {}  [38;2;100;210;255m{}\x1B[0m", f.title, url);
                            }
                            all_findings.push(f);
                        }
                    }
                ...
                for url in crawled_urls.iter().take(self.args.payload_limit) {
                    ...
                    if let Ok(findings) = xss_scanner.comprehensive_scan(url, &params).await {
                        for finding in findings {
                            let f = self.convert_finding(&finding);
                            if verbose {
                                println!("  XSS   {}  [38;2;100;210;255m{}\x1B[0m", f.title, url);
                            }
                            all_findings.push(f);
                        }
                    }
                        ...
                        if let Ok(results) = lfi_scanner.exploit_lfi(url, &param).await {
                            for result in results {
                                if result.success {
                                    ...
                                    if verbose {
                                        println!("  LFI   {} via param `{}`  [38;2;100;210;255m{}\x1B[0m", result.technique, param, url);
                                    }
                                    all_findings.push(f);
                                }
                            }
        ...
        if !self.completed_phases.contains(&"vuln".to_string()) {
            self.completed_phases.push("vuln".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        if self.completed_phases.contains(&"tls".to_string()) {
            println!("  {} Phase 'tls' already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"tls".to_string()) {
            if !excluded.contains(&"tls".to_string()) {
                ...
                for finding in tls_findings {
                    ...
                    println!("  {} {} [38;2;100;210;255m| {}\x1B[0m",
        ...
        if !self.completed_phases.contains(&"tls".to_string()) {
            self.completed_phases.push("tls".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        if self.completed_phases.contains(&"cors".to_string()) {
            println!("  {} Phase 'cors' already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"cors".to_string()) {
        ...
        if !self.completed_phases.contains(&"cors".to_string()) {
            self.completed_phases.push("cors".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        if self.completed_phases.contains(&"common".to_string()) {
            println!("  {} Phase 'common' already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else {
            ...
            if !excluded.contains(&"common".to_string()) {
                ...
                if let Ok(common_scanner) = CommonAppScanner::new(120) {
                    ...
                    for finding in common_findings {
                        ...
                        if verbose {
                            println!("  {} {} [38;2;100;210;255m{}\x1B[0m", fmt_sev_label(&sev), finding.title, finding.url);
                        }
        ...
        } // end else (phase 7 resume skip)
        // Checkpoint after Phase 7
        if !self.completed_phases.contains(&"common".to_string()) {
            self.completed_phases.push("common".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        if self.completed_phases.contains(&"filter".to_string()) {
            println!("  {} Phase 'filter' already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else {
            ...
            if !excluded.contains(&"filter".to_string()) {
                ...
                for url in &crawled_urls {
                    if let Ok(resp) = self.client.get(url).await {
                        ...
                        if verbose {
                            println!("  [38;2;100;210;255mScanning {} for secrets...\x1B[0m", url);
                        }
        ...
        // Instagram OSINT
        if self.completed_phases.contains(&"insta".to_string()) {
            println!("  {} Instagram OSINT already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"insta".to_string()) {
        ...
        // Checkpoint after Instagram
        if !self.completed_phases.contains(&"insta".to_string()) {
            self.completed_phases.push("insta".to_string());
            self.save_checkpoint(&all_findings).await;
        }

        // Session Hijack Testing
        if self.completed_phases.contains(&"session".to_string()) {
            println!("  {} Session test already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"session".to_string()) {
        ...
        if !self.completed_phases.contains(&"session".to_string()) {
            self.completed_phases.push("session".to_string());
            self.save_checkpoint(&all_findings).await;
        }

        // ML-Based Zero-Day Detection
        if self.completed_phases.contains(&"zeroday".to_string()) {
            println!("  {} ML zero-day detection already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"zeroday".to_string()) || self.args.zeroday {
            if !excluded.contains(&"zeroday".to_string()) {
        ...
        // Checkpoint after ML Zeroday
        if !self.completed_phases.contains(&"zeroday".to_string()) {
            self.completed_phases.push("zeroday".to_string());
            self.save_checkpoint(&all_findings).await;
        }

        // Static path scanning
        if self.completed_phases.contains(&"static".to_string()) {
            println!("  {} Static path scan already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"static".to_string()) {
            if !excluded.contains(&"static".to_string()) {
                let static_findings = self.scan_static_paths().await?;
                all_findings.extend(static_findings);
            }
        }
        // Checkpoint after Static
        if !self.completed_phases.contains(&"static".to_string()) {
            self.completed_phases.push("static".to_string());
            self.save_checkpoint(&all_findings).await;
        }

        // Agent-based parallel scan
        check_timeout!();
        if self.completed_phases.contains(&"agent".to_string()) {
            println!("  {} Agent scan already completed — skipping", tc("▶", LAVENDER_BLUE));
        } else if modules.contains(&"all".to_string()) || modules.contains(&"agent".to_string()) {
        ...
        // Checkpoint after Agent
        if !self.completed_phases.contains(&"agent".to_string()) {
            self.completed_phases.push("agent".to_string());
            self.save_checkpoint(&all_findings).await;
        }
        ...
        // Cap findings to prevent memory exhaustion
        all_findings.truncate(self.findings_cap);
        ...
        if self.args.output.is_some() {
            let target = self.args.target_url();
            let html_output = HtmlReport::generate_header("OXIDE Scan Report", target, "", "", 0);
    ...
    async fn crawl_phase(&mut self) -> Result<Vec<String>> {
        let result = if let Some(ref mut hc) = self.headless_crawler {
            println!("  {} {}",
                tc("◆", OSAKA_JADE_B),
                tc("Headless Chrome crawling enabled — JS engine rendering", LAVENDER_BLUE));
    ...
    fn contains_xss(body: &str, baseline_body: &str, payload: &str, status: u16) -> bool {
        ...
        // Also check that payload isn't HTML-encoded (not exploitable if encoded)
        if baseline_body.contains(payload) { return false; }
        if !body.contains(payload) { return false; }
        // Blackhat logic: if the payload is HTML-encoded, it's not exploitable XSS
        let encoded_payload = payload.replace('<', "&lt;").replace('>', "&gt;");
        if body.contains(&encoded_payload) && !body.contains(payload) {
            return false;
        }
        true
    }
    ...
    fn contains_lfi(body: &str, baseline_body: &str, status: u16) -> bool {
        ...
        let lfi_signals = [
            "root:x:0:0", "root:$1$", "/bin/bash", "/usr/sbin/nologin",
    ...
    async fn fuzz_url(&self, url: &str,
        ...
        // Extract CF bypass session for payload mutation and request headers
        let cf_session_guard = self.cf_session.lock().ok();
        let cf_session_data = cf_session_guard.as_ref().and_then(|g| g.as_ref().cloned());
        drop(cf_session_guard);
        ...
        let test_types = [
            ...
            for (mod_idx, &(label, payloads, count)) in test_types.iter().enumerate() {
                prog_mod.store(mod_idx, Ordering::SeqCst);
                for payload in payloads.iter().take(count) {
                    ...
                    let final_payload = if let Some(ref session) = cf_session_data {
                        if let Some(ref cf) = self.cf_bypass {
                            cf.mutate_payload(payload, session)
                        } else {
                            payload.to_string()
                        }
                    } else {
                    ...
                    let fuzz_url = UrlUtil::inject_param(url, param, &urlencoding::encode(&final_payload));
                    *prog_fuzz_url.lock().unwrap_or_else(|e| e.into_inner()) = fuzz_url.clone();
                    ...
                    let http_req = {
                        let mut req = crate::http::request::HttpRequest::get(&fuzz_url);
                        if let Some(ref session) = cf_session_data {
                            for (k, v) in &session.active_headers {
                                req.add_header(k, v);
                            }
                        }
                        req
                    ...
                    match response {
                        Ok(response) => {
                            ...
                            if let Some(ref cf) = self.cf_bypass {
                                if let Ok(mut guard) = self.cf_session.lock() {
                                    if let Some(ref mut session) = *guard {
                                        cf.update_session(session, status, &response.headers, &response.body);
                                    }
                                }
                            }
                    ...
                    yield_check!(findings, errors, requests);
                }
            }
        }
...
use crate::cli::display::{
    ...
    LIGHT_CYAN, LIGHT_CYAN_B, RP_GOLD,
};
