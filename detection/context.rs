...
/// UUID-like format ensures it never appears naturally in valid traffic.
...
impl ContextAnalyzer {
    /// Full pipeline: inject marker → find reflections → detect context → test filter → generate payloads.
    /// Returns `None` if the value is not reflected (no point testing further — FP = 0).
    pub fn analyze(
    ...
    /// Get the reflection marker for injection.
    pub fn marker() -> &'static str {
        REFLECT_MARKER
    }

    /// Get the char probe marker for character testing.
    pub fn char_probe_marker() -> &'static str {
        CHAR_PROBE_MARKER
    }

    /// Inject marker into URL for a given parameter.
    pub fn inject_marker(url: &str, param: &str) -> String {
        UrlUtil::inject_param(url, param, REFLECT_MARKER)
    }

    /// Inject char probe into URL.
