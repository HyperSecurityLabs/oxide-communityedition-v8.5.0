...
use crate::cli::display::{HISUI, SHU, YAMABUKI, TSUYUKUSA, KIKYO};
...
impl CmdInjectionScanner {
    ...
    pub async fn start_interactive_shell(&self, url: &str, param: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        println!("{}", tc("[+] Starting interactive shell session...", HISUI));
        ...
        println!("{}", format!("[+] Connected to {}@{}:{}",
            session.user.truecolor(TSUYUKUSA.0, TSUYUKUSA.1, TSUYUKUSA.2), url.truecolor(KIKYO.0, KIKYO.1, KIKYO.2), session.working_directory.truecolor(HISUI.0, HISUI.1, HISUI.2)));
        ...
        loop {
            ...
            if command == "exit" || command == "quit" {
                println!("{}", "[+] Exiting shell session".truecolor(YAMABUKI.0, YAMABUKI.1, YAMABUKI.2));
                break;
            }
            if command == "help" {
            ...
            if command.starts_with("cd ") {
                ...
                if let Ok(result) = self.change_directory(&session, new_dir).await {
                    if result.success {
                        ...
                        println!("{}", format!("Changed to: {}", session.working_directory).truecolor(HISUI.0, HISUI.1, HISUI.2));
                    } else {
                        println!("{}", "cd: No such file or directory".truecolor(SHU.0, SHU.1, SHU.2));
                    }
                }
                continue;
            }
            ...
            match self.execute_command(url, param, command).await {
                Ok(result) => {
                    if result.success {
                        ...
                        println!("{}", format!("Command failed: {}", command).truecolor(SHU.0, SHU.1, SHU.2));
                    }
                }
                Err(e) => println!("{}", format!("Error: {}", e).truecolor(SHU.0, SHU.1, SHU.2)),
            }
        }
    ...
    fn show_shell_help(&self) {
        println!("{}", "\n─── Interactive Shell Help ───".truecolor(KIKYO.0, KIKYO.1, KIKYO.2));
        ...
        println!("{}", "=============================".truecolor(KIKYO.0, KIKYO.1, KIKYO.2));
    }
        ...
        for command in commands {
            if let Ok(result) = self.execute_command(url, param, format!("which {}", command).as_str()).await {
                if result.success && !result.output.trim().is_empty() {
                    available_commands.push(format!("{}: {}", command.truecolor(HISUI.0, HISUI.1, HISUI.2), result.output.trim()));
                }
            }
        }
