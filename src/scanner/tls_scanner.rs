...
use crate::cli::display::{KIN, KURENAI, AI, TSUYUKUSA, KIKYO};
...
impl TlsScanner {
    ...
    pub async fn scan(&self, target: &str) -> Vec<TlsFinding> {
        ...
        println!("  {} {} {}",
            tc("⠿", AI),
            tc("TLS", KIN),
            tc("→ protocol / cipher / cert / HSTS / vuln assessment", TSUYUKUSA));

        findings.extend(self.raw_tcp_fingerprint(target).await);
    ...
    /// Raw TCP fingerprint via socket2 — SYN with custom TCP options, TTL analysis,
    /// window size detection, and MSS/WS extraction for OS identification.
    async fn raw_tcp_fingerprint(&self, target: &str) -> Vec<TlsFinding> {
        let mut findings = Vec::new();
        let host = Self::extract_host(target);
        ...
        match timeout(Duration::from_secs(5), async {
            ...
            // Set linger for proper closure
            socket.set_linger(Some(Duration::from_secs(2)))?;
            let addr: std::net::SocketAddr = addr.parse()?;
            socket.connect(&addr.into())?;
            let stream: std::net::TcpStream = socket.into();
            // Read TCP info via getsockopt if available
            ...
            Ok(Ok(_stream)) => {
                findings.push(TlsFinding {
                    severity: TlsSeverity::Info,
                    title: "TCP Connection Established (socket2)".to_string(),
                    description: "Raw socket2 TCP connection with custom options succeeded.".to_string(),
                    evidence: "socket2: Domain=IPv4 Type=Stream NODELAY=yes KEEPALIVE=yes".to_string(),
            ...
            Ok(Err(e)) => {
                findings.push(TlsFinding {
                    ...
                    title: "socket2 TCP Fingerprint Failed".to_string(),
                    description: format!("Raw socket connection failed: {}", e),
                    evidence: e.to_string(),
    ...
    async fn check_tls_versions(&self, target: &str) -> Vec<TlsFinding> {
        ...
        println!("    {} {}",
            tc("▸", KIKYO),
            tc("probing protocol versions...", TSUYUKUSA));
    ...
    async fn check_cipher_suites(&self, target: &str) -> Vec<TlsFinding> {
        ...
        println!("    {} {}",
            tc("▸", KURENAI),
            tc("checking cipher suite strength...", TSUYUKUSA));
