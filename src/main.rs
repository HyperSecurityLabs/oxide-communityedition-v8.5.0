...
async fn print_scan_info(args: &CliArgs) {
    ...
    use crate::cli::display::{
        HISUI_B, FUJI, KIKYO_B, TSUYUKUSA, KURENAI,
    };

    Output::print_header("Target Information");
    if args.multiattack_enabled() {
        println!("  {} {}  {} {}",
            tc("▸", HISUI_B), tc("Multi-Attack", KIKYO_B).bold(),
        ...
        for (i, url) in args.url.iter().enumerate() {
            ...
            let _host = url::Url::parse(&clean)
                ...
                .unwrap_or_default();
            println!("  {} {}  {} {}  {} {}",
                tc("▸", HISUI_B), tc(&format!("Target {}", i + 1), KIKYO_B).bold(),
                tc("→", FUJI), tc(&clean, FUJI),
                tc("≈", HISUI_B), tc(&format!("{} thr", per_target), HISUI_B));
        }
        println!("  {} {}  {} {}  {} {}",
            tc("▸", HISUI_B), tc("Threads", KIKYO_B).bold(),
            tc("→", FUJI), tc(&format!("{} total", args.threads), HISUI_B),
            tc("·", HISUI_B), tc(&format!("{}s duration", args.duration), FUJI));
    } else {
        ...
        let ip_display = if ips.is_empty() {
            tc("unresolved", KURENAI)
        } else {
            tc(&ips.join(", "), HISUI_B)
        };
        println!("  {} {}  {} {}",
            tc("▸", HISUI_B), tc("Target", KIKYO_B).bold(),
            tc("→", FUJI), args.target_url().truecolor(FUJI.0, FUJI.1, FUJI.2).bold());
        println!("  {} {}  {} {}",
            tc("▸", HISUI_B), tc("IP", KIKYO_B).bold(),
            tc("→", FUJI), ip_display);
        println!("  {} {}  {} {}  {} {}",
            tc("▸", HISUI_B), tc("Threads", KIKYO_B).bold(),
            tc("→", FUJI), tc(&args.threads.to_string(), HISUI_B),
            tc("·", HISUI_B), tc(&format!("{}s duration", args.duration), FUJI));
    }
    ...
    println!("  {} {}  {} {}",
        tc("▸", HISUI_B), tc("Modules", KIKYO_B).bold(),
        tc("→", FUJI), module_line.join(tc(" │ ", FUJI).as_str()));

    if let Some(output) = &args.output {
        println!("  {} {}  {} {}",
            tc("▸", HISUI_B), tc("Output", KIKYO_B).bold(),
            tc("→", FUJI), tc(output, HISUI_B));
    }

    if args.verbose  { println!("  {} {}", tc("▸", HISUI_B), tc("Verbose mode", HISUI_B)); }
    if args.insecure { println!("  {} {}", tc("▸", HISUI_B), tc("SSL verification disabled", SHU)); }
    if args.zeroday  { println!("  {} {}", tc("▸", HISUI_B), tc("Zero-day detection", HISUI_B)); }
    if args.train    { println!("  {} {}", tc("▸", HISUI_B), tc("Training mode", SHU)); }
    if args.insta    { println!("  {} {}", tc("▸", HISUI_B), tc("Instagram OSINT", TSUYUKUSA_B)); }
    if args.session  { println!("  {} {}", tc("▸", HISUI_B), tc("Session hijack testing", TSUYUKUSA)); }
        ...
        for header in &args.header {
            match Parser::parse_header(header) {
                Ok((key, value)) => println!("    {}: {}",
                    tc(&key, HISUI_B), tc(&value, KIKYO_B)),
...
async fn main() {
    ...
    if args.list_modules {
        ...
        println!("\n  {} Available modules:",
            "◆".truecolor(HISUI_B.0, HISUI_B.1, HISUI_B.2));
    ...
    if is_multi {
        println!("  {} {}  {} {}",
            tc("⚔", YAMABUKI),
            tc("Multi-Attack engaged", HISUI_B).bold(),
            tc("→", GIN),
            tc(&format!("{} concurrent targets", total_targets), KIKYO_B));
    }
    println!("  {} {}",
        tc("◈", YAMABUKI),
        tc("Launching scan — sit tight", KIKYO_B).bold());
    println!();
    ...
    if args.zeroday && !is_multi && !args.get_modules().contains(&"engine".to_string()) {
        println!("  {} {}",
            tc("◈", HISUI_B),
            tc("Zero-Day Standalone — isolated ML anomaly scanning", KIKYO_B).bold());
        println!();
        ...
        println!("\n  {} {}    {} {}",
            tc("·", HISUI_B), tc(&format!("Duration: {}", final_duration), TSUYUKUSA),
            tc("·", HISUI_B), tc(&format!("Ended: {}", TimeUtil::format_timestamp(&TimeUtil::now())), GIN));
    ...
    let (findings, hybrid_scanner, total_reqs) = if is_multi {
            ...
            match run_scan_cancellable(scanner.run_hybrid_scan()).await {
                Ok(f) => {
                    ...
                    println!("  {} {}  {}",
                        ...
                        tc(&format!("{} findings", f.len()), HISUI_B));
        ...
        println!("  {} {}", tc("⚔", YAMABUKI), tc(&summary, KIKYO_B));
    ...
    if findings.is_empty() {
        println!("  {} {}", tc("◈", HISUI_B), tc("No vulnerabilities found — target appears secure", TSUYUKUSA));
    } else {
        println!("  {} {}", tc("◈", YAMABUKI), tc(&format!("Found {} issue(s):", findings.len()), TSUYUKUSA));
        for f in &findings {
            Output::print_finding_stylish(
                &format!("{:?}", f.severity),
                &f.title,
        ...
        if !detailed_findings.is_empty() && args.verbose {
            ...
            for (idx, finding) in detailed_findings.iter().take(10).enumerate() {
                println!("    {}. {} — {}",
                    ...
                    tc(&finding.title, KIKYO_B),
                    tc(&finding.url[..finding.url.len().min(60)], TSUYUKUSA));
            }
