                ...
                if !self.args.verbose {
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ...
                            let det_s = if det > 0 {
                                tc(&format!("{} det", det), SHU)
                            } else {
                                tc("0 det", GIN)
                            };
                            let err_s = if err > 0 {
                                tc(&format!("{} err", err), HISUI)
                            } else {
                                tc("0 err", GIN)
                            };

                            let border = tc(&format!(" {}", "\u{2502}"), GIN);
                            ...
                            let url_display = if cur_fuzz.len() > 55 {
                                format!("{}…", &cur_fuzz[..cur_fuzz.floor_char_boundary(54)])
                            } else {
                                cur_fuzz.clone()
                            };
                            println!("\r\x1B[2K  {} {} {}",
                                tc("\u{25B6}", FUJI),
                                tc(&url_display, WAKABA),
                                border);

                            let status = format!("{} req  \u{00B7}  req/s:{:.0}", req, rate);
                            println!("\r\x1B[2K  {} {} {}",
                                tc("\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}", TSUYUKUSA),
                                tc(&status, TSUYUKUSA),
                                border);
