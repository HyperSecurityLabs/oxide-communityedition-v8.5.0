...
impl ZeroDayStandalone {
    ...
    pub async fn run(&mut self) -> Result<Vec<Finding>> {
        ...
        println!("  {} {}",
            tc("◈", HISUI_B),
            tc("[Zero-Day Standalone] Initialising ML anomaly engine...", KIKYO_B));

        println!("  {} Scanning {} with ML-based zero-day detection",
            tc("▸", TSUYUKUSA),
            tc(&self.target_url, FUJI));

        if let Some(limit) = self.duration_limit {
            println!("  {} Duration limit: {}s",
                tc("▸", TSUYUKUSA),
                tc(&limit.as_secs().to_string(), KIKYO_B));
        }
        println!();
        ...
        println!("  ── {} ──",
            tc("Phase 1/3: Reconnaissance — crawling target", TSUYUKUSA));
        ...
        println!();
        println!("  ── {} ──",
            tc("Phase 2/3: ML anomaly scanning — dynamic fuzzing", TSUYUKUSA));
        ...
        for url in &urls {
            ...
            print!("\r  {} [{}/{}] {:.0}%  |  {}  |  anomalies: {}  |  exploits: {}  |  remaining: {}   ",
                tc("▸", TSUYUKUSA),
                tc(&analyzed.to_string(), HISUI_B),
                tc(&total.to_string(), GIN),
                pct,
                tc(&truncate_url(url, 55), FUJI),
                tc(&self.total_anomalies.load(Ordering::Relaxed).to_string(), YAMABUKI),
                tc(&self.confirmed_exploits.load(Ordering::Relaxed).to_string(), HISUI_B),
                tc(&remaining, GIN));
        ...
        println!();
        println!("  ── {} ──",
            tc("Fuzz Phase: Random payload flood", TSUYUKUSA));
        ...
        for (fi, target) in fuzz_targets.iter().enumerate() {
            ...
            print!("  {} target {}/{}: {}  ",
                tc("▸", TSUYUKUSA),
                tc(&(fi + 1).to_string(), KIKYO_B),
                tc(&fuzz_targets.len().to_string(), GIN),
                tc(&truncate_url(target, 50), FUJI));
            use std::io::{Write, stdout};
            stdout().flush().ok();
            ...
            for (pi, (label, payload)) in fuzz_payloads.iter().enumerate() {
                ...
                let result = match tokio::time::timeout(Duration::from_secs(5), fetch).await {
                    Ok(Ok(resp)) => {
                        if resp.status >= 500 {
                            errors_5xx += 1;
                            format!("{} {}[500]", tc(label_str, YAMABUKI), tc("●", YAMABUKI))
                        } else {
                            String::new()
                        }
                    }
                    Ok(Err(_)) => {
                        crashed += 1;
                        format!("{} {}[CRASH]", tc(label_str, SHU), tc("●", SHU))
                    }
                    Err(_) => {
                        timeouts += 1;
                        format!("{} {}[TIMEOUT]", tc(label_str, YAMABUKI), tc("◌", TSUYUKUSA))
                    }
                };
        ...
        println!("  {} Fuzz results for {} tests across {} targets",
            tc("╔═══", TSUYUKUSA),
            tc(&total_fuzz_tests.to_string(), KIKYO_B),
            tc(&fuzz_targets.len().to_string(), GIN));
        println!("  {}  5xx errors:  {}  (server crashed or error page)",
            tc("║", fuzz_severity),
            tc(&errors_5xx.to_string(), if errors_5xx > 0 { YAMABUKI } else { HISUI_B }));
        println!("  {}  Timeouts:     {}  (request hung / no response)",
            tc("║", fuzz_severity),
            tc(&timeouts.to_string(), if timeouts > 0 { YAMABUKI } else { HISUI_B }));
        println!("  {}  Connection failures: {}  (connection refused / reset)",
            tc("║", fuzz_severity),
            tc(&crashed.to_string(), if crashed > 0 { SHU } else { HISUI_B }));
        ...
        println!();
        println!("  ── {} ──",
            tc("Phase 2.75: HTTP Parameter Pollution detection", TSUYUKUSA));
        ...
        match crate::zero_day::hpp::HppDetector::scan(
            ...
            Ok(hpp_summary) => {
                println!("  {} HPP results: {} vulnerable URLs, {} anomalies",
                    tc("✓", HISUI_B),
                    tc(&hpp_summary.vulnerable_count.to_string(), YAMABUKI),
                    tc(&hpp_summary.total_anomalies.to_string(), YAMABUKI));
            }
            Err(e) => {
        ...
        println!("  {} Requests sent: {}",
            tc("▸", GIN),
            tc(&total_reqs.to_string(), TSUYUKUSA));
        println!("  {} Anomalies detected: {}",
            tc("▸", GIN),
            tc(&total_anom.to_string(), YAMABUKI));
        println!("  {} Exploits confirmed: {}",
            tc("▸", GIN),
            tc(&confirmed.to_string(), HISUI_B));

        let rate_color = if success_rate >= 50.0 { HISUI_B }
            else if success_rate >= 20.0 { YAMABUKI }
            else { SHU };
        println!("  {} Zero-Day success rate: {}/{} ({:.1}%)",
            tc("◈", rate_color),
            tc(&confirmed.to_string(), rate_color),
            tc(&total_anom.to_string(), GIN),
            success_rate);
        ...
        if let Some(limit) = self.duration_limit {
            ...
            println!("  {} Duration used: {:.1}s / {}s ({:.0}%)",
                tc("▸", GIN),
                tc(&elapsed.as_secs_f64().to_string(), TSUYUKUSA),
                tc(&limit.as_secs().to_string(), GIN),
                used_pct);
        }
