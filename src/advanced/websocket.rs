                        ...
                        Err(e) => {
                            ...
                            if err_str.contains("timeout") {
                                findings.push(WebSocketFinding {
                                    ...
                                    description: format!("Server did not respond to payload: {}", &payload[..payload.floor_char_boundary(payload.len().min(50))]),
                                });
                            }
                        }
                    }
        ...
        for error in &sql_errors {
            if lower_resp.contains(error) {
                return Some(WebSocketFinding {
                    ...
                    description: format!("SQL error detected in response to: {}", &payload[..payload.floor_char_boundary(payload.len().min(50))]),
                });
            }
        }
                            ...
                            if !response.is_empty() && !response.contains("error") {
                                findings.push(WebSocketFinding {
                                    ...
                                    description: format!("Operation succeeded without auth: '{}' -> '{}'", 
                                        &msg[..msg.floor_char_boundary(msg.len().min(50))], 
                                        &response[..response.floor_char_boundary(response.len().min(100))]),
                                });
                            }
                        }
